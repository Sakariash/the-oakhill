    ...
    <div {...storyblokEditable(blok)} className="relative w-72 h-96 bg-black rounded-3xl overflow-hidden cursor-pointer transition-transform duration-250 hover:scale-90 group">
      ...
      <div className="absolute inset-0 z-10">
        <Image
          ...
          layout="fill"
          objectFit="cover"
          className="transition-transform duration-250 group-hover:scale-125"
        />
      </div>
      <div className="absolute inset-0 bg-black opacity-0 transition-opacity duration-250 group-hover:opacity-50 z-20"></div>
      
      <div className="absolute w-full bottom-0 left-0 z-30 text-white p-5 pb-8 flex flex-col space-y-2 transform translate-y-2/4 group-hover:translate-y-0 transition-transform duration-1000">
        <h2 className="text-2xl text-left">
          {blok.headline}
        </h2>
        {hasInfo && (
          <p className="text-base text-left w-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000">
            {blok.info}
          </p>
        )}
        {hasButton && (
          <button className="mt-2 py-1 px-4 bg-white border-2 text-black border-black rounded-lg transition-all duration-1000 hover:bg-violet-300 hover:text-black self-end opacity-0 group-hover:opacity-100">
            See more
          </button>
        )}
      </div>
